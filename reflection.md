...
- Describe one moment where you did not accept an AI suggestion as-is.  
    **Response:** There were some time where the AI will suggest logic that does not connect with the whole scope of the project and imply that it is needed. Without providing additional context, it will just output unwanted code. Having to explain the reason of the code helps me determine if it will be refactors to connect with the other logic. 

- How did you evaluate or verify what the AI suggested?  
    **Response:**  Have it explain the function of the code, provide any feedback if there is any other ways of handling or simplifying the code. I did noticed that simplifying does not mean it will be readable for others and me. 
...
- Why were these tests important?  
    **Response:** These will help narrow down possibilities that prevent user error and verify the expected behaviors are working without second guessing the output. Example, the conflict test will create a duplicate to confirm that the system can detect it to warn the user to resolve it before it causes any missed care. 

**b. Confidence**

- How confident are you that your scheduler works correctly?  
    **Response:** Planning test cases will reduce the time needed to ship the application. 

- What edge cases would you test next if you had more time?  
    **Response:** 
        - If recurring task data is invalid for add_task (negative cost/zero duration), verify system behavior explicitly.
        - Tasks with empty time should always appear at end, even mixed with invalid/non-empty values.
        - Priority sorting with unknown/messy priority values
...
- What part of this project are you most satisfied with?  
    **Response:** System design of the project, test case did handle well. Creating the structure of the application was handled ok. 

**b. What you would improve**

- If you had another iteration, what would you improve or redesign?  
    **Response:** While the app was working, seems that copilot failed to implement the features to the UI and missed multiple features. I had to switch to Claude to check for missing UI/UX, well, it has changed a lot. 
...
My experience with using Copilot seems that the task requested went through but when I ran the streamlit, there was multiple features that were missing. Having to switch to Claude did changed a lot of code but provide a reason why it was needed. 